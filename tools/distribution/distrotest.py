...
class DistroTest(object):
    ...
    def __init__(self, checker: checker.Checker, path: str, installable: str, name: str, image: str, tag: str):
    ...
    @cached_property
    def image(self) -> DockerTestImage:
    ...
    async def exec(self, container: aiodocker.containers.DockerContainer) -> int:
        ...
        if _out:
            if return_code:
                ...
                if not self.exiting and not "distros" in self.errors:
...
class DebDockerTestImage(DockerTestImage):
...
class RPMDockerTestImage(DockerTestImage):
